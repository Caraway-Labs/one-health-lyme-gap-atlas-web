...
function AtlasPage() {
  ...
  const filtered = useMemo(() => (scoresQuery.data?.counties ?? []).filter((county) => {
    ...
    const district = HEALTH_DISTRICTS[stateFilter]?.find((item) => item.id === districtFilter);
    return county.in_contiguous_tick_scope && (stateFilter === "ALL" || county.state === stateFilter) && (!district || district.counties.includes(county.county)) && (!needle || `${county.county} ${county.state} ${county.fips}`.toLowerCase().includes(needle)) && matchesEvidence(county, evidence);
  }), [scoresQuery.data, stateFilter, districtFilter, query, evidence]);
  ...
  }, [metadataQuery.data?.release_id, stateFilter, districtFilter, query, evidence, selectedFips, settings, router]);
