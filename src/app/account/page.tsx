...
const roles: { value: Exclude<UserProfileWriteRole, null>; label: string }[] = [
  ...
  { value: "district_level_epidemiologist", label: "District-level epidemiologist" },
  { value: "state_level_epidemiologist", label: "State-level epidemiologist" },
  { value: "state_director_level_epidemiologist", label: "State director-level epidemiologist" },
  { value: "national_level_epidemiologist", label: "National-level epidemiologist" },
];
  ...
  useEffect(() => {
    async function loadAccount() {
      try {
        ...
        setSignedIn(true); setLoadingProfile(true);
        const result = await getProfileV1MeProfileGet();
        if (result.status === 200 && result.data.profile) setForm(result.data.profile);
      } catch { setNotice("Your profile is temporarily unavailable. You can continue exploring Atlas."); }
