...
function validConversation(value: unknown, now: number): value is LocalConversation {
...
export function loadConversations(now = Date.now()): LocalConversation[] {
  ...
  try {
    const parsed = JSON.parse(localStorage.getItem(CHAT_STORAGE_KEY) ?? "{}") as Partial<ChatStore>;
    const conversations = (parsed.conversations ?? [])
      .filter((item) => validConversation(item, now))
      .sort((a, b) => Date.parse(b.updatedAt) - Date.parse(a.updatedAt))
      .slice(0, MAX_CONVERSATIONS);
    if (conversations.length !== (parsed.conversations ?? []).length) saveConversations(conversations);
...
export function saveConversations(conversations: LocalConversation[]): void {
  const value: ChatStore = { version: 1, conversations: conversations.slice(0, MAX_CONVERSATIONS) };
  localStorage.setItem(CHAT_STORAGE_KEY, JSON.stringify(value));
  window.dispatchEvent(new Event(CHAT_STORAGE_EVENT));
}

export function createConversation(response: KnowledgeChatResponse, question: string): LocalConversation {
  const now = new Date();
  return {
