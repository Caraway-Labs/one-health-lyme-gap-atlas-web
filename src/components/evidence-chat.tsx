  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const updated: LocalConversation = {
        ...
        turns: [
          ...conversation.turns,
          { id: `${response.request_id}:user`, role: "user", text: question, createdAt: now },
          { id: response.request_id, role: "assistant", text: response.answer, response, createdAt: now },
        ],
      };
      const next = [updated, ...conversations.filter((item) => item.id !== updated.id)].slice(0, 5);
      saveConversations(next);
      setConversations(next);
      setActiveId(updated.id);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Evidence chat is unavailable.");
    } finally {
      setPending(false);
    ...
    <div className={`evidence-chat evidence-chat-${mode}`}>
      {mode === "workspace" && (
        <aside className="chat-history" aria-label="Local conversation history">
          <div><strong>Recent chats</strong><button type="button" onClick={() => { clearConversations(); setConversations([]); setActiveId(""); }}>Clear all</button></div>
          {conversations.length === 0 ? <p>No saved conversations yet.</p> : conversations.map((item) => (
            <div className="history-row" key={item.id}>
              <button type="button" aria-current={active?.id === item.id} onClick={() => setActiveId(item.id)}>{item.title}</button>
              <button type="button" aria-label={`Delete ${item.title}`} onClick={() => deleteOne(item.id)}>×</button>
            </div>
          ))}
          <p className="retention-copy">Up to five conversations are stored in this browser for 30 days. Deleting here removes the local copy only; the server copy expires under its 30-day retention policy.</p>
        </aside>
      )}
      <section className="chat-panel" aria-label="Ask the evidence">
        <header>
          <div><span className="kicker">Reviewed literature</span><h1>{mode === "drawer" ? "Ask the evidence" : "Knowledge graph evidence workspace"}</h1></div>
          <button className="button secondary" type="button" onClick={() => setActiveId("__new__")}>New chat</button>
        </header>
        <p className="medical-notice">{publicCopy.medical_notice}</p>
        <div className="chat-transcript" aria-live="polite">
          {!active?.turns.length && <div className="chat-empty"><strong>Start with a research question</strong><p>Ask about surveillance, vectors and hosts, environmental exposure, diagnostics, interventions, or outcomes.</p></div>}
          {active?.turns.map((turn) => (
            <article className={`chat-turn ${turn.role}`} key={turn.id}>
              <strong>{turn.role === "user" ? "You" : "Evidence assistant"}</strong>
              <p>{turn.text}</p>
              {turn.response?.citations?.length ? <ol className="citation-list">{turn.response.citations.map((citation) => (
                <li key={citation.citation_id}><a href={citation.pubmed_url} target="_blank" rel="noopener noreferrer">{citation.title} (PMID {citation.pmid})</a><small>{citation.source_label}</small></li>
              ))}</ol> : null}
            </article>
          ))}
          {pending && <p role="status" className="processing">Searching reviewed evidence…</p>}
          {error && <p role="alert" className="chat-error">{error} Try again in a moment.</p>}
        </div>
