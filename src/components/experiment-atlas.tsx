...
export function ExperimentAtlas({ variant }: ExperimentProps) {
  ...
  return <main className={`experiment experiment-${variant}`}>
    ...
    {detail ? <><VariantBody variant={variant} detail={detail} counties={filtered} geometry={geometryQuery.data} selectedFips={selectedFips} comparison={comparison} step={step} showTable={showTable} onStep={setStep} onSelect={setSelectedFips} onComparison={setComparisonFips} onToggleTable={() => setShowTable((open) => !open)} /><ScoringAssumptions detail={detail} settings={settings} onChange={setSettings} /></> : <section className="experiment-card"><p>Loading the selected county…</p></section>}
