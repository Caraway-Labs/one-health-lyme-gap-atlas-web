...
| Tracking-plan control | Register this allowlist in Amplitude Data (or an equivalent versioned schema) and generate/check the TypeScript event wrapper in CI. A PR that adds an event or property must update this document, schema, tests, privacy inventory, and reviewer approval. |
